...
public class Movies extends JPanel implements ListSelectionListener {
	...
	public Movies(int movieID) {
		...
		// Populate the movie information by looking up the supplied movie ID in
		// the database.
		try {
			...
			ResultSet rs = stmt
					.executeQuery("select * from film_text where film_id="
							+ movieID);
			rs.next();
				...
				public void actionPerformed(ActionEvent event) {
					...
					try {
						con = DriverManager.getConnection(
								...
								"");
						Statement stmt = con.createStatement();
						ResultSet colcount = stmt
								.executeQuery("select count(title) from Arkansas_Queue");
						colcount.next();
						int number = colcount.getInt(1);
						number = number + 1;
						stmt.executeUpdate("insert into Arkansas_Queue (position, title) values ("
								+ number + " ,'" + title + "')");
			...
			removeFromQueue.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent event) {
					// Clear the list model so that it ONLY shows the new
					// results set.
					java.sql.Connection con;
					try {
						con = DriverManager.getConnection(
			...
			ResultSet isMovieInQueue = stmt
					.executeQuery("select * from Arkansas_Queue where title='"
							+ title + "'");

			// If the movie is in the queue, only display the remove button.
			// Otherwise, only display the add button.
			...
			rs = stmt.executeQuery("select * from film_list where title='"
					+ title + "'");
			rs.next();
			...
			String[] actorList = rs.getString("actors")
					.replaceAll("^[,\\s]+", "").split(",");
			...
			actorInfo.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent e) {
					...
					String firstName = selectedItem.substring(0,
							selectedItem.indexOf(" "));
					String lastName = selectedItem.substring(
					...
					try {
						...
						Statement stmt = con.createStatement();
						ResultSet rs = stmt
								.executeQuery("select * from actor_info where (first_name='"
										+ firstName
										+ "'"
										+ "AND last_name='"
										+ lastName + "')");
